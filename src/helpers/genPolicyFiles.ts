...
export const genPolicyFiles: genPolicyFiles = (policyData, projectData) => {
    ...
    const policyOptions: object = { ...policyData.policy?.defaultOptions, ...projectData.policy?.options };

    for (let [relPath, { generate }] of policyData.genFiles.entries()) {
        try {
            generateFiles.set(relPath, generate(projectData.packageJson, policyOptions));
        } catch (error) {
